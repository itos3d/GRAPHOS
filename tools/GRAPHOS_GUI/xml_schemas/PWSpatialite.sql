...
CREATE TABLE pw_matches (
  ...
  id_keypoint_first INTEGER,
  id_keypoint_second INTEGER,
  enabled INTEGER,
  accuracy DOUBLE,
  FOREIGN KEY(id_matches_category) REFERENCES pw_matches_categories(id) ON DELETE CASCADE,
  FOREIGN KEY(id_keypoint_first) REFERENCES pw_keypoints(id) ON DELETE CASCADE,
  FOREIGN KEY(id_keypoint_second) REFERENCES pw_keypoints(id) ON DELETE CASCADE,
  PRIMARY KEY (id)
);

SELECT InitSpatialMetadata();
SELECT AddGeometryColumn('pw_keypoints', 'pixel_point_geom',-1, 'POINT', 'XY');
