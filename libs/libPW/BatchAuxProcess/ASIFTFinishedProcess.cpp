...
*-------------------------------------------------
*/
#include "ASIFTFinishedProcess.h"
#include "PWMatchesCategory.h"
#include "MicMac/TapiocaIO.h"
using namespace PW;

ASIFTFinishedProcess::ASIFTFinishedProcess(PW::Project *project,QString tiepointsMetadata):mProject(project),mTiepointsMetadata(tiepointsMetadata)
{

}

ASIFTFinishedProcess::~ASIFTFinishedProcess()
{

}

void ASIFTFinishedProcess::run(){
    mProject->setMatchingFormat(MATCHING_FORMAT_APERO_TXT);
    mProject->appendTiePointsSet("ASIFT");
    ...
    TapiocaIO tapiocaIO;
    tapiocaIO.readImagePairs(mProject->getBasePath()+"/"+MATCHING_PATH+"/ASIFT", imagePairs);
    ...
    if (!QFileInfo(mProject->getImagesFileName().first()).suffix().contains("tif")) {
        QString matchesDirPath = mProject->getBasePath()+"/"+MATCHING_PATH+"/ASIFT";
